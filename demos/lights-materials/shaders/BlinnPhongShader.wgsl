...
VertexOutput {
    ...
    output.normalWV = (camera.viewMatrix * normalWorld).xyz; //transform the normal to view space
...
fn fragmentMain(@location(1) normal: vec3f, @location(2) texCoord: vec2f, @location(3) positionWV: vec3f) -> @location(0) vec4f {
    ...
    if(directionalLight.enabled > 0){
        var l = ComputeDirectionalLight(directionalLight, material, normalize(normal), normalize(viewDirection.xyz), 1);
    ...
    if(pointLight.enabled > 0){
        var l = ComputePointLight(pointLight, material, positionWV, normalize(normal), normalize(viewDirection.xyz), 1);
        lighting.diffuse += l.diffuse;
        lighting.specular += l.specular;
        lighting.ambient += l.ambient;
    }
    ...
    if(spotLight.enabled > 0){
        var l = ComputeSpotLight(spotLight, material, positionWV, normalize(normal), normalize(viewDirection.xyz), 1);
        lighting.diffuse += l.diffuse;
        lighting.specular += l.specular;
        lighting.ambient += l.ambient;
    }
